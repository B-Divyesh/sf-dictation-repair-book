...
function chrome(content: string, title: string, kicker: string) {
  ...
  return `<div class="app-shell" data-theme="${state.settings.theme}" data-release-tag="${releaseTag}" data-release-commit="${releaseCommit}">
    ...
    ${isDemo() ? `<footer class="demo-site-footer"><p>Private rules for repaired dictation text.</p><nav aria-label="Legal"><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a></nav><small>${releaseTag} · Built by Param Factory</small></footer>` : ''}
  </div>`;
}
...
function settingsPage() {
  return chrome(`<div class="settings-stack">
    <section class="settings-section"><div><p class="eyebrow">Approved sources</p><h2>Application access</h2><p>Labels are a consent boundary and are stored locally. Clipboard capture still happens only after your click.</p></div><div><ul class="app-list">${state.apps.map((a) => `<li><label><input type="checkbox" data-app-toggle="${a.id}" ${a.enabled ? 'checked' : ''}><span>${esc(a.name)}</span></label><button class="icon-button danger" data-app-delete="${a.id}" aria-label="Remove ${esc(a.name)}">×</button></li>`).join('')}</ul><form id="add-app-form" class="inline-form"><label class="sr-only" for="new-app">New application</label><input id="new-app" name="name" required maxlength="50" placeholder="Add application"><button class="button secondary">Add</button></form></div></section>
    <section class="settings-section"><div><p class="eyebrow">Interoperability</p><h2>Take your words anywhere</h2><p>Exports are always available, even without a paid license.</p></div><div class="button-grid"><button class="button secondary" data-action="export-csv">Export CSV</button><button class="button secondary" data-action="export-json">Back up JSON</button><button class="button secondary" data-action="export-whisper">Copy Whisper prompt</button><label class="button secondary file-button">Import JSON<input id="import-json" type="file" accept="application/json,.json"></label></div></section>
    <section class="settings-section"><div><p class="eyebrow">Appearance</p><h2>Theme</h2></div><fieldset class="segmented"><legend class="sr-only">Color theme</legend>${['system','light','dark'].map((t) => `<label><input type="radio" name="theme" value="${t}" ${state.settings.theme === t ? 'checked' : ''}><span>${t[0].toUpperCase()+t.slice(1)}</span></label>`).join('')}</fieldset></section>
    <section class="settings-section license"><div><p class="eyebrow">One-time unlock</p><h2>${nativeSampleMode ? 'Sample purchases are disabled' : unlocked ? 'Unlimited book active' : 'Keep an unlimited repair book'}</h2><p>${nativeSampleMode ? 'The sample cannot add, remove, or verify a license. Start for real to manage purchases.' : unlocked ? 'This device has a valid license. Verification is cached for one day.' : 'Free includes 25 approved rules, testing, and every export. Pay $12 once for unlimited approved rules.'}</p><p><a href="https://dictation-repair-book.sociobot.in/privacy">Privacy</a> · <a href="https://dictation-repair-book.sociobot.in/terms">Terms</a></p></div><div>${nativeSampleMode ? '<span class="stamp">SAMPLE ONLY</span>' : unlocked ? `<span class="stamp success">LICENSE ACTIVE</span><button class="button text-button" data-action="remove-license">Remove from device</button>` : `<a class="button primary" href="${checkoutUrl}" rel="external">Buy $12 license on Sociobot checkout (opens Sociobot checkout)</a><form id="license-form"><label for="license-token">Have a license? Paste it</label><div class="inline-form"><input id="license-token" name="token" required autocomplete="off"><button class="button secondary">Verify</button></div></form>`}</div></section>
    <section class="settings-section danger-zone"><div><p class="eyebrow">Delete local data</p><h2>${nativeSampleMode ? 'Reset this sample repair book' : 'Erase this repair book'}</h2><p>${nativeSampleMode ? 'Restores the shipped sample. Your encrypted vault and license data are untouched.' : isNative() ? 'Removes the encrypted vault, its local key, and license data.' : 'Removes this browser repair book and its license data.'} Export first if you want a copy.</p></div><button class="button danger-button" data-action="${nativeSampleMode ? 'reset-demo' : 'erase'}">${nativeSampleMode ? 'Reset sample data' : 'Erase all local data'}</button></section>
...
async function persist(message = '') {
  try {
    if (mayTouchRealNativeData(nativeSampleMode)) await saveState(state);
...
app.addEventListener('click', async (event) => {
  ...
  if (action === 'remove-license' && mayTouchRealNativeData(nativeSampleMode)) { clearLicense(); unlocked = false; notice = 'License removed from this device.'; render(); }
  if (action === 'erase' && mayTouchRealNativeData(nativeSampleMode) && confirm(`Erase all ${state.corrections.length} corrections, application labels, and license data from this device? This cannot be undone.`)) { await eraseVault(); clearLicense(); state = emptyState(); unlocked = false; notice = 'All local repair-book and license data erased.'; page = 'capture'; render(); }
  ...
  if (action === 'load-sample' && isNative()) { nativeSampleMode = true; unlocked = false; state = sampleState(); proposal = null; testResult = null; page = 'rules'; notice = 'Sample repair book loaded. It is separate until you keep it.'; render(true); }
  if (action === 'keep-sample' && nativeSampleMode) { nativeSampleMode = false; unlocked = cachedUnlock(); await persist('Sample repair book saved as your local repair book.'); }
...
app.addEventListener('submit', async (event) => {
  ...
  if (form.id === 'test-form') { testResult = applyRules(String(data.get('input')), approved()); for (const rule of state.corrections) if (testResult.applied.includes(rule.intended)) rule.hits++; if (mayTouchRealNativeData(nativeSampleMode)) await saveState(state); render(); }
  if (form.id === 'license-form' && mayTouchRealNativeData(nativeSampleMode)) { storeLicense(String(data.get('token'))); const verdict = await verifyLicense(true); unlocked = verdict.valid; notice = verdict.valid ? 'License verified. Unlimited rules are active.' : verdict.reason === 'offline' ? 'Could not reach verification. Your last valid status is unchanged.' : verdict.reason === 'rate_limited' ? 'Verification is busy. Wait a moment before trying again; your last valid status is unchanged.' : 'That license is not active for this product.'; render(); }
});
