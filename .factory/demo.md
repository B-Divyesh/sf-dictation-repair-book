...
- `met a pro lol` → `metoprolol` from Notes
- `cube or net ease` → `Kubernetes` from VS Code
- `Neem` → `Niamh` from VS Code
...
The installed app also offers **Load sample repair book** on its first-run Capture screen. That sample stays in memory and is not written to the encrypted vault until the person chooses **Keep this repair book**. Its reset, license, and erase controls cannot touch the real vault or license data. **Start for real** discards it.
